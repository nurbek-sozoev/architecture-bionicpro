...
def update_etl_metadata(**context):
    execution_date = context['execution_date']
    report_date = (execution_date - timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    records_processed = daily_result[0][0] if daily_result else 0
    
    query = """
        INSERT INTO analytics.etl_metadata (dag_id, last_processed_date, records_processed)
        VALUES ('transform_load_dag', %(report_date)s, %(records_processed)s)
    """
    
    execute_clickhouse_query(query, {'report_date': report_date, 'records_processed': records_processed})
    print(f"Updated ETL metadata: last_processed_date={report_date}, records_processed={records_processed}")
    
    return f"ETL metadata updated for {report_date}"
...
with DAG(
    ...
    update_metadata_task = PythonOperator(
        ...
        python_callable=update_etl_metadata,
    )
    
    [wait_crm, wait_telemetry] >> build_daily_task >> build_summary_task >> verify_task >> optimize_task >> update_metadata_task
